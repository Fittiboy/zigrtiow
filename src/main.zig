...
pub fn main() !void {
    ...
    const materials = [_]Material{
        Material.lambertian(Vec3.init(0, 0, 1)),
        Material.lambertian(Vec3.init(1, 0, 0)),
    };
    const MatCount = RefCounted(Material);
    const mat_refs = try allocator.alloc(MatCount, 2);
    ...
    const R = std.math.cos(pi / 4);

    const left = Hittable.initSphere(.{ -R, 0, -1 }, R, mat_refs[0]);
    defer left.deinit();
    const right = Hittable.initSphere(.{ R, 0, -1 }, R, mat_refs[1]);
    defer right.deinit();
    try world.add(left);
    try world.add(right);
